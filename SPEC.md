...
设计状态：已确认  
实现状态：核心模块及真实微信文本收发已验证；微信入站媒体链路已实现，真实微信媒体、离开状态主动推送与最终实机抢占仍待验收  
日期：2026-07-16
...
| `/ok <n>` | 批准 Bridge 发起的单次审批 |
| `/no <n>` | 拒绝 Bridge 发起的单次审批 |
| `/help` | 显示唯一命令表 |

不提供中文命令、别名、隐藏管理命令或 `/stop`。
...
4. 通知回复窗口从完成或失败通知成功送达起 5 分钟有效；Desktop 审批通知不创建回复窗口。
...
- Codex App Server 最终回复没有可信的媒体输出字段，因此当前出站只发送文本，不支持 Codex → 微信附件或语音；不得从助手自然语言中猜测本地路径并自动外发文件。
...
- 在场：Windows 未锁屏且最近 5 分钟有键鼠输入。
- 离开：锁屏或连续 5 分钟无键鼠输入。
- 微信发起的回合无论用户是否在场，都把最终回复发送到微信。
- Desktop 发起的普通完成通知只在离开时推送。
- Desktop 失败或等待审批在离开时推送；在场时由 Desktop 呈现。
- 成功送达的 Desktop 完成或失败通知创建 5 分钟通知回复窗口；通知中明确显示项目和会话标题。
...
6. 锁屏或 5 分钟空闲后，Desktop 后台任务完成会推送微信；用户在场时不重复推送。
7. 活动任务期间系统不睡眠，结束后恢复电源策略。
8. Bridge App Server 审批可通过 `/ok`、`/no` 处理并在 30 分钟后自动拒绝；Desktop 审批只通知不远程处理。
9. Bridge/Windows 重启后恢复游标、绑定、文本/附件队列和 Outbox；中断回合不重复执行。
...
24. Codex 最终文本中出现本地路径不会触发附件外发；当前 Codex → 微信仍严格为文本回复。
...
8. **已实现，待长期实机验收**：Presence、离开通知、通知送达后 5 分钟回复路由与活动任务电源保持。
