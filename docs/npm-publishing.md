...
本文档面向 Codex iLink 的维护者，覆盖 npm 包和 Windows 独立 exe。普通用户只需参考 README 的安装章节。
...
先在只安装 Node.js 22（最低 22.13）的干净 Windows 环境验收 npm 包。
...
`artifacts/` 应同时包含 exe 和 `.exe.sha256`，两者必须匹配。

SEA 注入后不能沿用 Node.js 原始签名。公开稳定版必须在注入完成后使用项目自己的 Authenticode 证书签名，再重新生成 `.sha256`；`Get-AuthenticodeSignature` 的结果应为 `Valid`。

当前工作流在未配置签名凭证时产出未签名预览版，Windows SmartScreen 可能显示“未知发布者”。签名证书属于外部发布凭证，不应提交到仓库。

推送版本标签会触发 `.github/workflows/release.yml`，在干净的 Windows runner 上重新测试和构建：
...
工作流会创建或更新对应的 GitHub Release，并上传 exe 与校验文件。
