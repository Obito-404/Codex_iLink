...
Bridge App Server 自己只有一个实时待审批时由 `y` 或 `n` 决定；多个待审批时可使用 `y<code>`、`n<code>` 逐条处理。`ya`、`na` 只展示并快照当前清单，用户必须在两分钟内回复带唯一确认码的 `ya#<code>`、`na#<code>` 才批量处理该快照；切换操作或任何其他消息会取消确认，之后新增的请求不继承批量决定。审批短码和批量确认码随机生成、在进程内不复用，旧回复不能命中新请求；即使预览延迟送达，也不能批准被后续命令替换的新快照。微信通知发送失败不会伪装成用户拒绝，而是在请求仍存活时使用同一短码退避重试；请求超时或 App Server 回调丢失时明确通知失效，Bridge 重启后旧短码也无法批准。回调本身不持久化，避免重启后批准已经不存在的请求。
