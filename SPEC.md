...
- Codex App Server 最终回复没有可信的媒体输出字段，因此 Bridge 只把独占一行的标准 Markdown Windows 本地文件链接视为显式出站附件；兼容 `C:\...` 与 `C:/...`，不得从普通自然语言路径、HTTP 链接或非独占行链接猜测并外发文件。图片、视频和普通文件按官方媒体流程发送，语音气泡不支持。
